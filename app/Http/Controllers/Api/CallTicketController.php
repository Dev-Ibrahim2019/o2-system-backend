...
class CallTicketController extends ApiController
{
    ...
    public function manual(Request $request):JsonResponse{$data=$request->validate(['phone'=>['required','string','max:40'],'branch_id'=>['nullable','integer','exists:branches,id'],'external_call_id'=>['nullable','string','max:255']]);$data['branch_id']=$request->user()->hasRole('super-admin')?($data['branch_id']??$request->user()->branch_id):$request->user()->branch_id;$data['agent_id']=$request->user()->id;return $this->success('Call ticket opened',$this->tickets->openManualTicket($data));}
