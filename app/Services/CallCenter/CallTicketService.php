...
class CallTicketService
{
    ...
    public function completeTicket(CallTicket $ticket,User $agent,?string $disposition=null,?string $notes=null,?string $callType=null):CallTicket{$attributes=['status'=>'completed','ended_at'=>now(),'duration_seconds'=>max(0,now()->diffInSeconds($ticket->answered_at??$ticket->started_at)),'disposition'=>$disposition,'notes'=>$notes??$ticket->notes];if($callType!==null)$attributes['call_type']=$callType;return $this->transition($ticket,$agent,$attributes);}
    ...
    public function classifyTicket(CallTicket $ticket,User $agent,string $callType):CallTicket{return $this->transition($ticket,$agent,['call_type'=>$callType]);}
    public function rateTicket(CallTicket $ticket,User $agent,int $rating,?string $feedback=null):CallTicket{return $this->transition($ticket,$agent,['satisfaction_rating'=>$rating,'satisfaction_feedback'=>$feedback]);}
